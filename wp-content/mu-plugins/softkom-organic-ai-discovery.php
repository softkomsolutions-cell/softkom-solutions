<?php
/**
 * Plugin Name: Softkom Organic + AI Search Discovery
 * Description: Crawlable organic and AI-search discovery layer for the Softkom assessment.
 */
if ( ! defined( 'ABSPATH' ) ) { exit; }

function softkom_organic_is_assessment() {
    if ( is_admin() ) { return false; }
    if ( function_exists( 'is_page' ) && is_page( 'assessment' ) ) { return true; }
    $path = wp_parse_url( isset( $_SERVER['REQUEST_URI'] ) ? wp_unslash( $_SERVER['REQUEST_URI'] ) : '', PHP_URL_PATH );
    if ( ! is_string( $path ) ) { return false; }
    return (bool) preg_match( '#/assessment/?$#', $path );
}
...
function softkom_organic_source() {
    ...
    foreach ( $map as $needle => $value ) { if ( $ref && false !== strpos( $ref, $needle ) ) { return $value; } }
...
function softkom_organic_assessment_schema() {
    ...
    $graph = array(
        ...
        '@graph' => array(
            array('@type'=>'Organization','@id'=>home_url('/#organization'),'name'=>'Softkom Solutions','url'=>home_url('/'),'description'=>'South African business systems, AI automation and digital transformation consultancy.','areaServed'=>array('@type'=>'Country','name'=>'South Africa')),
            array('@type'=>'WebPage','@id'=>$url.'#webpage','url'=>$url,'name'=>'Free Business Systems & AI Readiness Assessment | South Africa','description'=>'A free 3-minute assessment for South African businesses to identify AI automation, workflow, sales and business-system opportunities.','about'=>array('AI automation','business process automation','custom business systems','sales automation','South Africa')),
            array('@type'=>'Service','@id'=>$url.'#service','name'=>'Business Systems & AI Readiness Assessment','provider'=>array('@id'=>home_url('/#organization')),'areaServed'=>array('@type'=>'Country','name'=>'South Africa'),'serviceType'=>'AI automation and business systems assessment','url'=>$url),
...
function softkom_organic_assessment_markup() {
    ...
    <section id="softkom-organic-discovery" aria-labelledby="softkom-organic-title" style="max-width:1120px;margin:48px auto 72px;padding:0 24px;color:#1E293B;font-family:Inter,system-ui,sans-serif;box-sizing:border-box;">
      <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:20px;padding:32px;">
        ...
        <h3 style="color:#0F172A;">What you receive</h3><p>A business-systems maturity score, an AI and automation opportunity score, prioritised recommendations and a practical next step based on your answers. The assessment takes about three minutes.</p>
        <h3 style="color:#0F172A;">Built for South African businesses</h3><p>Recommendations focus on measurable outcomes such as time saved, faster sales response, better customer experience, cleaner data and scalable processes.</p>
      </div>
...
function softkom_organic_append_to_assessment_content( $content ) {
    if ( ! softkom_organic_is_assessment() || ! in_the_loop() || ! is_main_query() ) { return $content; }
    if ( false !== strpos( $content, 'id="softkom-organic-discovery"' ) ) { return $content; }
