...
function softkom_qa_result($checks,$started){
    ...
    foreach($checks as $c){
        $level=isset($c['level'])?$c['level']:'check';
        if('warning'===$level){if(empty($c['ok']))$warnings++;continue;}
        !empty($c['ok'])?$passed++:$failed++;
    }
    return array('ran_gmt'=>current_time('mysql',true),'started_gmt'=>$started,'passed'=>$passed,'failed'=>$failed,'warnings'=>$warnings,'checks'=>$checks,'status'=>$failed?'FAIL':'PASS');
}
function softkom_qa_check(&$checks,$ok,$label,$detail='',$level='check'){$checks[]=array('ok'=>(bool)$ok,'label'=>$label,'detail'=>$detail,'level'=>$level);}
...
function softkom_qa_run(){
    ...
    try{
        ...
        if(function_exists('softkom_search_money_slugs')){$search_slugs=softkom_search_money_slugs();softkom_qa_check($checks,count(array_intersect($slugs,(array)$search_slugs))===count($slugs),'Search discovery cluster contains all 9 pages');}else softkom_qa_check($checks,false,'Search discovery cluster loaded');
        ...
        $idx=(int)get_option('softkom_indexnow_last_code',0);softkom_qa_check($checks,in_array($idx,array(200,202),true),'IndexNow last response accepted',(string)$idx,'warning');
        $idx_urls=get_option('softkom_indexnow_last_urls',array());softkom_qa_check($checks,count((array)$idx_urls)===9,'IndexNow last submission contains 9 URLs',(string)count((array)$idx_urls),'warning');
        $last_lead_ids=get_posts(array('post_type'=>'softkom_lead','post_status'=>'any','posts_per_page'=>1,'fields'=>'ids','orderby'=>'date','order'=>'DESC'));softkom_qa_check($checks,is_array($last_lead_ids),'Lead repository query succeeds');
...
function softkom_qa_admin_page(){
    ...
    if($r){$status=esc_html($r['status']);$warnings=isset($r['warnings'])?(int)$r['warnings']:0;echo '<h2>Status: '.$status.'</h2><p>Last run (GMT): '.esc_html($r['ran_gmt']).' &nbsp; Passed: '.(int)$r['passed'].' &nbsp; Failed: '.(int)$r['failed'].' &nbsp; Warnings: '.$warnings.'</p><table class="widefat striped" style="max-width:1100px"><thead><tr><th>Result</th><th>Check</th><th>Detail</th></tr></thead><tbody>';foreach((array)$r['checks'] as $c){$level=isset($c['level'])?$c['level']:'check';$result=!empty($c['ok'])?'PASS':('warning'===$level?'WARN':'FAIL');echo '<tr><td>'.esc_html($result).'</td><td>'.esc_html($c['label']).'</td><td>'.esc_html($c['detail']).'</td></tr>';}echo '</tbody></table>';}
    else echo '<p>No automated QA run has completed yet.</p>';
    echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'" style="margin-top:20px"><input type="hidden" name="action" value="softkom_qa_run_now">';wp_nonce_field('softkom_qa_run_now');submit_button('Run Safe QA Now','primary','submit',false);echo '</form><p><strong>IndexNow:</strong> availability/submission status is informational and cannot fail the core funnel QA. Do not use the manual IndexNow submission control on production while the host-specific admin-post issue is unresolved.</p><p><strong>Deep transactional QA:</strong> continue using the existing WP-CLI runtime smoke suite before deployment. It creates isolated test records and cleans them up outside the public admin request.</p></div>';
}
