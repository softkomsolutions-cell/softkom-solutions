...
function softkom_indexnow_slugs(){
    ...
    return array('assessment','ai-automation-south-africa','business-process-automation-south-africa','custom-business-systems-south-africa','ai-automation-for-smes-south-africa','replace-spreadsheets-manual-processes-south-africa','sales-lead-generation-automation-south-africa','whatsapp-customer-service-automation-south-africa','ai-readiness-assessment-south-africa','replace-excel-with-custom-software-south-africa','custom-software-vs-spreadsheets','automate-manual-business-processes','business-system-integration-south-africa','automate-data-entry-south-africa','automate-business-reporting-south-africa','automate-approvals-workflows-south-africa','connect-business-software-south-africa','automate-lead-follow-up-south-africa','operations-management-system-south-africa');
}
...
function softkom_indexnow_diagnostics_page(){
    ...
    echo '<form method="post" action="" style="margin-top:20px"><input type="hidden" name="softkom_indexnow_submit_all" value="1">';wp_nonce_field('softkom_indexnow_submit_all');echo '<input type="hidden" name="softkom_indexnow_submit_token" value="'.esc_attr(softkom_indexnow_queue_token()).'">';submit_button('Queue All 19 Acquisition URLs','primary','submit',false);echo '</form><p>This Tools-page action verifies permissions and a CSRF nonce, then only queues a background submission. No long-running outbound IndexNow HTTP request happens inside the browser request, and refreshing cannot double-submit.</p></div>';
}
