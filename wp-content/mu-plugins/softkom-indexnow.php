...
function softkom_indexnow_post_saved($post_id,$post,$update){
    ...
    if(!in_array($post->post_name,softkom_indexnow_slugs(),true))return;
    $url=get_permalink($post_id);if(!$url)return;
    wp_schedule_single_event(time()+30,'softkom_indexnow_submit_event',array(array($url)));
}
...
add_action('init',function(){
    $version='1.0.0';if(get_option('softkom_indexnow_cluster_version')===$version)return;
    $urls=array();foreach(softkom_indexnow_slugs() as $slug){$page=get_page_by_path($slug,OBJECT,'page');if($page&&'publish'===$page->post_status)$urls[]=get_permalink($page);}
