...
function softkom_attribution_capture(){
 if(is_admin()||wp_doing_ajax())return;$path=(string)wp_parse_url(wp_unslash($_SERVER['REQUEST_URI']??''),PHP_URL_PATH);if(!preg_match('#/(assessment|ai-automation-south-africa|business-process-automation-south-africa|custom-business-systems-south-africa)/?$#',$path))return;
 ...
 /* Keep first meaningful touch; direct can be upgraded by a later identifiable source. */
...
function softkom_attribution_persist_lead($lead_id){
 ...
 /* Stable first-touch fields protect attribution if later funnel code updates compatibility fields. */
