...
function softkom_assessment_runtime_render() {
    ...
    ob_start();
    include $template;
    if ( function_exists( 'softkom_organic_assessment_content' ) ) {
        softkom_organic_assessment_content();
    }
    return ob_get_clean();
}
...
function softkom_assessment_runtime_assets() {
    ...
    if ( is_readable( $css ) ) {
        wp_enqueue_style( 'softkom-assessment-runtime', $base_url . '/softkom-assessment.css', array( 'softkom-assessment-runtime-fonts' ), (string) filemtime( $css ) );
    }
    if ( is_readable( $js ) ) {
